...
The generated candidates are not directly shippable as full replacement frames:
...
They are still useful as controlled material because the sleeve masks are coherent and close to the desired moderate broad-sleeve target.
...
The normalized and projected outputs are useful as local sleeve-shape material. The `0.7008` and `0.8519` non-sleeve drift values prove the full generated frames must not replace shipped frames directly, while the sleeve material can still be adopted through deterministic post-processing.
...
The production frames are still generated by the deterministic Reimu slicer and post-processing path. The OpenAI candidate is now a measured local sleeve-mask/proportion input that feeds `reimu_openai_sleeve_material_recipe.json`, not a shipped full-body replacement.
