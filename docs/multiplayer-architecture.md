...
- When Trystero reports `limited` or startup falls to `offline`, the Room exposes `Retry mesh` to recreate the transport without a full page reload. Diagnostics are mirrored through `data-room-transport-p2p`, `data-room-mesh-retryable`, and `data-room-mesh-retry-count`.
