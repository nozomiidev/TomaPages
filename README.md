...
The Reimu asset audit checks frame count, margins, detached alpha fragments, thin detached alpha slivers, line-like internal holes, weak alpha, expression-state center/size spread, and neighboring 5x5 direction center steps. Detached fragments, thin detached slivers, and line-like internal holes are gated at zero by default because they usually indicate disconnected appendages, floating contour residue, or transparent interior strokes rather than intentional hand or sleeve parts. The line-like hole detector includes small holes and longer high-aspect transparent slits so narrow cutout scars do not slip just above the old pixel-area threshold. The audit also reports transparent RGB residue for forensic cleanup, but does not gate on that WebP decoder-dependent value by default. The issue overlay colors internal transparent holes red, detached alpha fragments blue, and weak-alpha pixels yellow so questionable frames can be reviewed without guessing from CSV numbers alone.
